...
export async function dashboardView({ root }) {
  ...
  const allShiftsRaw = await getShifts(activeStation.id);
  const expenseByShift = await getExpensesByShift(activeStation.id);
  const allShifts = allShiftsRaw.map(s=>({ ...s, fin: computeShiftFinancials(s, expenseByShift[s.id]||0) }));
  ...
  todayShiftsMy.forEach(s=>{ 
    ...
    varianceMy += s.fin.variance;
    if (s.fin.toCollect > 0.5) toHandoverMy += s.fin.toCollect;
  });
