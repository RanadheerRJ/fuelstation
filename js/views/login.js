...
export async function loginView({ root }) {
    ...
    root.innerHTML = `
      <div class="login-wrapper">
        ...
        <div class="login-card">
          ...
          <p style="text-align:center;color:var(--text-secondary);font-size:14px;margin:8px 0 20px">Create developer account</p>
          <div id="setupAlert"></div>
          <div class="grid" style="gap:16px">
            <div><label class="label">Name</label><input id="setup_name" class="neu-input" placeholder="Your name" autocomplete="name"></div>
            <div><label class="label">Phone</label><input id="setup_phone" class="neu-input" type="tel" placeholder="+91 98765 43210" autocomplete="tel"></div>
            <div><label class="label">PIN (4 digits)</label><input id="setup_pin" class="neu-input" type="password" inputmode="numeric" maxlength="4" placeholder="••••" autocomplete="new-password"></div>
  ...
  // iOS Simple Login - NO numpad, NO pin dots, just normal inputs you can type
  root.innerHTML = `
    <div class="login-wrapper">
      <div class="login-brand">
        <div class="brand-mark">F</div>
      ...
      <div class="login-card">
        <h2 style="font-size:22px;font-weight:700;text-align:center;letter-spacing:-0.02em">Login</h2>
        ...
        <div class="grid" style="gap:18px">
          <div>
            ...
            <input id="phoneInput" class="neu-input" type="tel" placeholder="+91  00000 00000" inputmode="tel" autocomplete="tel" autofocus />
          </div>

          <div>
            <label class="label">4-digit PIN</label>
            <input id="pinInput" class="neu-input" type="password" inputmode="numeric" pattern="[0-9]*" maxlength="4" placeholder="••••" autocomplete="current-password" />
            <p style="font-size:11px;color:var(--text-tertiary);margin-top:6px">Enter your 4-digit PIN</p>
          </div>

          <button id="loginBtn" class="neu-btn neu-btn--primary neu-btn--block" style="margin-top:8px">Login</button>

          <p style="text-align:center;font-size:13px;color:var(--text-secondary);margin-top:4px">No account? Contact developer to get invited.</p>
        </div>
      </div>

      <p style="font-size:11px;color:var(--text-tertiary);margin-top:20px;text-align:center">Simple • Clean • iOS Style</p>
    </div>
  `;
  ...
  async function doLogin() {
    ...
    if (!phone) { showAlert('Enter phone number'); phoneInput.focus(); return; }
    if (!pin || pin.length!==4 || !/^\d{4}$/.test(pin)) { showAlert('Enter valid 4-digit PIN'); pinInput.focus(); return; }
